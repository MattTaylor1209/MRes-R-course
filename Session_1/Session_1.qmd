...
format: 
  ...
  pdf:
    ...
    include-in-header:
      text: |
        \usepackage{fvextra}
        % Keep Pandoc's highlighting command chars AND wrap long lines
...
Before we start, there's a small bit of setup code we need to run to make sure everything we need is installed. To do this, click "File -\> Open File..." and load the file called "setup.R".

Click the "Source" button in the top right ![](images/clipboard-3683134815.png){width="105"}

Now you can close that file.

Also, before we start, click on the little gear above the document window and select "clear all output" from the drop-down menu (this prevents any *spoilers!*):
...
Throughout this course, we’ll be using **Quarto**, a modern, flexible system for creating dynamic documents with embedded R code. (If you’ve ever used R Markdown or Jupyter before, Quarto will feel familiar). Quarto lets us write code, text, and create plots all in one place, making it easy to produce reproducible reports, figures, and even interactive documents.

You don’t need to know anything about Quarto to get started—we’ll walk through the basics together. For now, all you need to know is that your `.qmd` file acts as your lab notebook (and please use it as a notebook!): you can write plain English alongside R code, and Quarto will render it into a nicely formatted document with just one click.
...
**These documents are *yours*.** Treat them like you would any set of lecture notes (except way more interactive and useful). Write notes on them using comments like this ![](images/clipboard-3055594606.png){width="219"}. <!--# this is a comment! --> Rephrase things in your own way. Add chunks of code if you have an idea and want to test it. It's a *living document* and it's here to help you learn in whatever way you find easiest. You can always re-download the original from my GitHub if you accidentally delete something important!
...
-   ***R for Data Science(2nd Edition)*** **—** Hadley Wickham & Garrett Grolemund. A free and highly readable introduction to the tidyverse approach to R (and everyone's go-to when they try to teach themselves R). This course is based on theirs but highly abbreviated due to time constraints, so it is worth checking out their original course for more in depth info. <https://r4ds.hadley.nz/intro.html>
...
-   ***Harvard Chan Bioinformatics core training modules*** — Harvard. Numerous self-paced modules from basic introductions to R through to complex bioinformatics analyses. <https://hbctraining.github.io/main/>
...
And you can access parts of a matrix using row and column indices using **square brackets**:
...
**We will come to more powerful uses of square brackets to access parts of matrices in *Session 3.***
...
The other thing we need is a collection of **packages** called the **tidyverse**, and some other additional packages. An R **package** is a collection of functions, data, and documentation that extends the capabilities of base R. Using packages is key to the successful use of R.
...
We have already installed the tidyverse in our setup code, but the code you would use to install this (or any other package) is a single line: `install.packages("tidyverse")`
...
***Note: we have already installed these in our setup so there is no need to run the following code right now.***

```{r other-packages}
#| eval: false
install.packages(
  c("tidyverse","arrow","babynames","curl","ggrepel","ggridges","ggthemes","hexbin","janitor","leaflet","nycflights13","openxlsx","palmerpenguins","plotly","repurrrsive","tidymodels","writexl","AnnotationDbi", "BiocManager","BiocVersion","DESeq2","lattice","Matrix","tidySummarizedExperiment","org.Rn.eg.db","GO.db","limma","edgeR","Rsubread","biomaRt","pheatmap","clusterProfiler"
  ))
```
...
One of the most useful tools in the tidyverse is the **pipe**, which we will go into more detail about in *Session 2*. Think of it as "take what is on the left and feed it into the function on the right." Its symbol looks like this `%>%` although you might also see it written elsewhere as `|>`. We will be using this *a lot* in future sessions, but a quick example now, suppose you wanted to count how many different animals of each species of penguin are in our dataset. You might need to open the resulting table by clicking ![](images/clipboard-2688435829.png){width="45" height="29"}
...
While scatterplots can show the relationship between 2 variables, they may mask the contribution of other variables to the relationship. Remember our earlier question: *"**Does the relationship vary by location or species?"***

We can incoporate this into our plot by modifying our aesthetics mapping to include this information. **Pause for a moment: how could we represent species on our plot without adding another axis or splitting the plot in any way?**
...
When a categorical variable is mapped to an aesthetic, `ggplot2` will automatically assign a unique value of the aesthetic (here a unique colour) to each unique level of the variable (each of the three species), a process known as **scaling—**we will learn more about scaling in the following sessions. `ggplot2` will also add a legend that explains which values correspond to which levels.

Now let’s add one more layer: a smooth curve displaying the relationship between body mass and flipper length. **Pause point**: **refer back to the code above, and think about how we can add this to our existing plot. Is it an aesthetic, or a new geom? If you have a** [cheat sheet](https://posit.co/wp-content/uploads/2022/10/data-visualization-1.pdf) **open, can you find the right one?**
...
Our "goal" plot ([Overall aim]) only had one line going through the whole dataset.
...
3.  `geom_point()`: "make points which have `flipper_length_mm` on the x axis, `body_mass_g` on the y axis, and are `coloured` by the `species`."
...
**How can we add the `shape`** **aesthetic?**
...
penguins %>% 
  ggplot(mapping = aes(x = flipper_length_mm, y = body_mass_g)) +
  geom_point(mapping = aes(colour = species, YOUR_CODE_HERE)) + 
  geom_smooth(method = "lm")
```
...
penguins %>% 
  ggplot(mapping = aes(x = flipper_length_mm, y = body_mass_g)) +
  geom_point(mapping = aes(colour = species, shape = species)) + # added shape into the geom_point aesthetics
  geom_smooth(method = "lm")
```
...
The manual way would be to specify the factor levels ourselves. *Note that here I am using the base R operator `$`* *which is useful if we want to quickly select a specific column in a dataset (here selecting the `species` column in `penguins`*:
...
You can see our x-axis label has changed to `fct_infreq(species)` matching what we had put in the aesthetic mapping for x `aes(x = fct_infreq(species))`. **How could we change this label back to `species`?**
...
We can use the `labs()` layer I introduced you to a moment ago:
...
*Note: in a Quarto document, when you make 2 or more plots within the same chunk, they are presented in different tabs. There are other ways you can present your plots (e.g., side-by-side, over-under) using the* [`patchwork`](https://patchwork.data-imaginist.com/) *package—see that link for details.*
...
I’ve also adjusted the line thickness with the `linewidth` argument to help the curves stand out more clearly against the background.

In addition, we could map `species` to both the `colour` and `fill` aesthetics, and controlled the transparency of the filled density curves with the `alpha` aesthetic. `alpha` takes values from 0 (fully transparent) to 1 (fully opaque); here it’s set to 0.5.
...
penguins %>% 
  ggplot(mapping = aes(x = flipper_length_mm, y = bill_length_mm)) +
  geom_point(mapping = aes(YOUR_CODE_HERE)) + 
  YOUR_CODE_HERE
```
...
We can see from this that Chinstrap penguins seem to be different from the other 2 groups—they have shorter flippers but longer bill lengths, suggesting their bill length is proportionally greater than expected from the overall relationship between flipper length and bill length.

*Adelie penguin*

![](images/clipboard-1340690039.png){width="385"}

*Chinstrap penguin*

![](images/clipboard-3781245489.png){width="773"}

*Gentoo penguin*

![](images/clipboard-1494466610.png){width="580"}
...
You can split your plot into small panels based on a variable using `facet_wrap()`. Just write `~` followed by the variable name (for example, `~island`). The variable you use must be categorical, because each category will get its own panel.
...
We will cover the different options we have available in R for saving plots more during the following sessions. But if you want to save a plot as an image in your current working directory, you can use the `ggsave()` function, which will save the plot most recently created to disk:
...
If you don’t specify the `width` and `height` they will be taken from the dimensions of the current plotting device. For reproducible code, you’ll want to specify them by using `width=`, `height=` and specifying the units using `units=` (which can take the inputs `"in"`, `"cm"`, `"mm"` or `"px"`). You can learn more about `ggsave()` in the Help section or by using `help(ggsave)`.
