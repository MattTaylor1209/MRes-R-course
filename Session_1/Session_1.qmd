...
One of the most useful tools in the tidyverse is the **pipe**, which we will go into more detail on later. Think of it as "take what is on the left and feed it into the function on the right." Its symbol looks like this `%>%` although you might also see it written elsewhere as `|>`. We will be using this *a lot* in future sessions, but a quick example now, suppose you wanted to count how many different animals of each species of penguin are in our dataset.
...
Can even do this for multiple variables combined together. **Before you run the following code, what do you predict will be the result?**
...
In the next session, we will start learning how to import, tidy, and manipulate data, focusing on the types of bioinformatics data you will encounter in a bulk RNAseq experiment.
