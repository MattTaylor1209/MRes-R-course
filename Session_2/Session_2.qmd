...
We've been ignoring the output of this till now, but let's just quickly look at what this is telling us. (You may not see this depending on when you installed R and ran the setup code).
...
With a tibble, you can also just preview it in Quarto by writing the object name in a code box and running the box:
...
`dplyr`’s functions are organized into four groups based on what they operate on: **rows**, **columns**, **groups**, or **tables**. We will cover these one by one!
...
Another mistake is writing “or” statements like you would in English. We might read the following as "filter the airport names to only include them if they are Birmingham or Stansted." With some variables, this might not throw an error (but still won't give you what you meant). In our case it does:
...
**Have a practice at filtering the data by other variables of your choosing. If you can't remember what the variables are called, use `names(uk_flights)`**:

```{r student-practice-1}
#| error: true
...
`select()` removes all of the other columns you haven't selected. If you want to keep them all but just rename a few, use `rename()`:
...
If you have a bunch of inconsistently named columns and it would be painful to fix them all by hand, remember `janitor::clean_names()` provides some useful automated cleaning ([Reading data from a file]).
...
There is no set rule about which you choose. You may decide that actually you prefer intermediate objects or nesting functions—that is completely up to you. But for me, the pipe makes so much more sense, so that is the style used throughout this course.
...
```{r plot-london-official}
# Example - ranking and plotting London Area Airports by percentage of flights which were classed as Official
uk_flights %>% 
  filter(reporting_airport_group_name == "London Area Airports") %>% 
  mutate(official_percentage = official / grand_total * 100) %>% 
  select(reporting_airport_name, official_percentage) %>% 
  ggplot(aes(
    x = reorder(reporting_airport_name, official_percentage), # reordering lowest to highest
    y = official_percentage
  )) +
  geom_bar(stat = "identity") + # telling geom_bar to use the x value as identity
  labs(x = "Airport name", y = "Percentage of flights which were Official")
```
...
You can create groups using more than one variable. Our data doesn't work particularly well in this example, so let's make a new variable first. We'll call an airport "busy" if it saw more than 5000 flights in this reporting period. Because we actually want to add this as a column to our data, we need to use the assignment operator:
...
```{r message-summarising-multiple-grouped-variables}
busy_n <- busy_areas %>%
...
busy_n <- busy_areas %>%
  summarise(number_of_airports = n(),
            avg_flights = mean(grand_total, na.rm = TRUE),
            .groups = "drop_last")
...
If you want to combine some of the above steps (grouping and then summarising) into one, `dplyr` also includes an experimental feature called `.by`. Say you want to summarise the mean number of flights, grouping both by the area of the airport and the "busy-ness":
...
For now, the information we require is contained within the `stat` component. One highly useful operator in R is the `$` sign, as this allows you to select different components within objects. Note: you can also use `$` to select specific columns in data frames as we did earlier—I have not covered this in much detail as we are following a tidyverse-centric approach to working with data frames, but this is still a very useful feature in the right context.
